...
async function cmdFavorites(positional: string[], flags: Record<string, string | boolean>, format: OutputFormat) {
  ...
  if (subcommand === "save") {
    ...
    try {
      // Resolve folder: if --folder given, find by name or ID; otherwise default (first folder)
      ...
      if (folderArg) {
        ...
        // Default folder = first one (usually "Merkliste")
        const folders = await listFolders();
        if (folders.length === 0) {
...
// ─── Vehicle Search (car, moto, van, caravan) ──────────────────────────

async function cmdVehicleSearch(
  command: string,
  positional: string[],
  flags: Record<string, string | boolean>,
  format: OutputFormat,
) {
  // Subvertical is the command itself
  const subvertical = command;

  // Keyword is the positional args joined
  ...
  try {
    const result = await searchVehicles(subvertical, {
    ...
    if (format === "text") {
      ...
      for (const [k, v] of Object.entries(result.resolvedFilters)) {
      ...
      for (const item of items) {
        ...
        // Build detail line based on vehicle type
...
function cmdHelp(format: OutputFormat) {
  const help = {
    ...
    searchFlags: [
      ...
      { flag: "--fuel <type>", desc: "Vehicle: diesel/benzin/elektro/hybrid" },
      { flag: "--transmission <type>", desc: "Vehicle: automatik/schaltgetriebe" },
      { flag: "--type <type>", desc: "Vehicle: enduro/chopper/SUV/kombi/etc." },
      { flag: "--year-from <year>", desc: "Vehicle: min Erstzulassung" },
      { flag: "--year-to <year>", desc: "Vehicle: max Erstzulassung" },
      { flag: "--max-km <km>", desc: "Vehicle: max mileage" },
    ...
    examples: [
      ...
      "whcli seller abc123",
      "",
      "# Vehicles (car / moto / van / caravan)",
      "whcli moto enduro --max-price 3000 --text",
      "whcli moto --type chopper --fuel benzin --sort price-asc --text",
      "whcli car golf --fuel diesel --max-price 5000 --location wien --text",
      "whcli car --type 'SUV / Geländewagen' --year-from 2015 --private --text",
      "whcli caravan --max-price 20000 --text",
      "whcli van --fuel diesel --max-price 10000 --text",
